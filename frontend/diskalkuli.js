...
let dogruSayac = 0;
let yanlisSayac = 0;
let ustUsteYanlis = 0; // Adaptif öğrenme: Zorluğu düşürmek için kullanılır
...
function yeniSoruUret() {
  ...
  // Adaptif Öğrenme: Çocuk üst üste 2 kez yanlış yaparsa,
  // zorluk seviyesi otomatik olarak en kolaya (sadece ufak sayılarla toplama) çekilir.
  if (ustUsteYanlis >= 2) {
    islemler = ['toplama'];
    maksSayi = 3;
  }
  ...
  if (islem === 'toplama') {
    const a = rastgeleSayi(1, maksSayi);
    const b = rastgeleSayi(1, maksSayi);
    soru.a = a;
    soru.b = b;
    soru.cevap = a + b;
...
function cevapVerildi(secilenDeger, tiklananBtn) {
  ...
  if (dogruMu) {
    ...
    dogruSayac++;
    ustUsteYanlis = 0; // Başarı durumunda zorluk normale döner
  } else {
    ...
    yanlisSayac++;
    ustUsteYanlis++; // Hata sayacı artar

    // Eğer çocuk zorlanıyorsa teşvik edici bir mesaj ekle
    if (ustUsteYanlis >= 2) {
      geriBildirimEl.textContent += " (Biraz daha kolaylaştıralım! 🎈)";
    }
  ...
  // Her 10 soruda bir skoru backend'e kaydet (düzenli aralıklarla gönderim)
  if (skoruGonderGerekiyorMu() && typeof skorKaydet === 'function') {
...
window.addEventListener('pagehide', () => {
  if ((dogruSayac + yanlisSayac) === 0) return;
