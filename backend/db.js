...
function readDB() {
  ...
  try {
    ...
    const veri = JSON.parse(content);
    // Eski veritabanı dosyalarında sonradan eklenen alanlar eksik olabilir, tamamla
    for (const anahtar of Object.keys(defaultData)) {
      if (veri[anahtar] === undefined) veri[anahtar] = defaultData[anahtar];
    }
    for (const anahtar of Object.keys(defaultData.nextIds)) {
      if (veri.nextIds[anahtar] === undefined) veri.nextIds[anahtar] = defaultData.nextIds[anahtar];
    }
    return veri;
  } catch (err) {
...
// ─── ONEMLI GUNLER ──────────────────────────────────────────────────────────
function getOnemliGunler(profilId) {
  const db = readDB();
  return db.onemli_gunler
    .filter(g => g.profil_id === Number(profilId))
    .sort((a, b) => new Date(a.tarih) - new Date(b.tarih));
}

function insertOnemliGun({ profilId, baslik, tarih }) {
  const db = readDB();
  const id = db.nextIds.onemli_gunler++;
  const yeniGun = {
    id,
    profil_id: Number(profilId),
    baslik,
  ...
  db.onemli_gunler.push(yeniGun);
  writeDB(db);
  return yeniGun;
}

function deleteOnemliGun(id) {
  const db = readDB();
  const index = db.onemli_gunler.findIndex(g => g.id === Number(id));
  if (index === -1) return false;

  db.onemli_gunler.splice(index, 1);
  writeDB(db);
  return true;
}

// ─── PLANLANMIŞ GÖREVLER (AI'ın önerdiği saatli plan maddeleri) ─────────────
function getPlanlanmisGorevler(profilId) {
  const db = readDB();
  return db.planlanmis_gorevler
    .filter(g => g.profil_id === Number(profilId))
    .sort((a, b) => (a.tarih + a.saat).localeCompare(b.tarih + b.saat));
}

function insertPlanlanmisGorev({ profilId, tarih, saat, etkinlik, sure }) {
  const db = readDB();
  const id = db.nextIds.planlanmis_gorevler++;
  const yeniMadde = {
    id,
    profil_id: Number(profilId),
    tarih,
  ...
  db.planlanmis_gorevler.push(yeniMadde);
  writeDB(db);
  return yeniMadde;
}

function deletePlanlanmisGorev(id) {
  const db = readDB();
  const index = db.planlanmis_gorevler.findIndex(g => g.id === Number(id));
  if (index === -1) return false;

  db.planlanmis_gorevler.splice(index, 1);
  writeDB(db);
  return true;
}
...
module.exports = {
  ...
  insertOnemliGun,
  deleteOnemliGun,
  getPlanlanmisGorevler,
  insertPlanlanmisGorev,
  deletePlanlanmisGorev
};
